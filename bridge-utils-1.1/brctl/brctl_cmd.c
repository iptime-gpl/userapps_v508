...
#ifdef USE_PORT_BASED_VLAN_IN_BRIDGE
static int br_cmd_setportmap(int argc, char *const* argv)
{
	int map, err;

	if (sscanf(argv[3], "%x", &map) != 1) {
		fprintf(stderr, "bad port map value\n");
		return 1;
	}

	err = br_set_port_map(argv[1], argv[2], map);
	if (err)
		fprintf(stderr, "set port map failed: %s\n",
			strerror(errno));
...
{
#ifdef USE_PORT_BASED_VLAN_IN_BRIDGE
	printf("bridge name\tbridge id\t\tSTP enabled\tinterfaces\tportmap\n");
#else
...
#define CHUNK 128
	...
	for (i = 0; i < offset; i++) {
		...
		printf("%3i\t", f->port_no);
#ifdef USE_EFM_PRODUCT_XXX
		p = br->firstport;
		while (p != NULL) {
			if (p->index == f->port_no) break;
			p = p->next;
		}
		printf("%s\t", (p)?if_indextoname(p->ifindex, ifname):"");
#endif
...
#ifdef USE_PORT_BASED_VLAN_IN_BRIDGE
	{ 3, "setportmap", br_cmd_setportmap,
	  "<bridge> <port> <portmap>\tset port map" },
#endif
};

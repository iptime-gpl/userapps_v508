/* $Id: iptcrdr.c,v 1.6 2017/04/25 04:30:31 sgqca Exp $ */
/* MiniUPnP project
...
addnatrule(int proto, unsigned short eport,
           ...
           const char * rhost, char *nat_chain);
static int
				   ...
				   const char * desc, unsigned int timestamp)
{
	...
	while (strcmp(nat_chain_name[n], ""))
	{
		if (!strcmp(nat_chain_name[n], "upnp_app_postroute"))
			...
			r = addnatrule(proto, eport, iaddr, iport, rhost,  nat_chain_name[n]);
		n++;
	}	
#else
...
#ifdef USE_NF_CT_MAN_PROTO_32BIT
...
static int __sub_delete_redirect_and_filter_rules(
	char *chain_name, unsigned short eport, int proto, unsigned int *iaddr, unsigned short *iport)
{
	int r = -1;
	int i = 0;
	unsigned index = 0;
...
#ifdef IPTABLES_143
	for(e = iptc_first_rule(chain_name, h); e; e = iptc_next_rule(e, h), i++)
#else
	for(e = iptc_first_rule(chain_name, &h); e; e = iptc_next_rule(e, &h), i++)
#endif
	{
		if(proto==e->ip.proto)
		{
			...
			if(0 == strncmp(match->u.user.name, "tcp", IPT_FUNCTION_MAXNAMELEN))
			{
				...
				if (!strcmp(chain_name, "upnp_app_postroute"))
				{
					syslog(LOG_NOTICE, "APP_POSTROUTE(TCP) : addr(%08x, %08x), iport(%d,%d)",
						*iaddr, e->ip.dst.s_addr, *iport, info->dpts[0]);

					if((*iaddr != e->ip.dst.s_addr) || (*iport != info->dpts[0])) 
						continue;
				}
				else
				{
			...
			if (!strcmp(chain_name, "upnp"))
			{
				*iaddr = mr->range[0].min_ip;
#ifdef USE_NF_CT_MAN_PROTO_32BIT
				*iport = ntohs(mr->range[0].min.tcp.port);
#else
				*iport = ntohl(mr->range[0].min.all);
#endif
				syslog(LOG_NOTICE, "IADDR : %08x, IPORT : %d", *iaddr, *iport);
			}
			...
			{
			char desc[128];
			get_redirect_desc(eport, proto, desc, 128, NULL);
			if ((strstr(desc, "ipDISK_Server_FTP") || strstr(desc, "ipdisk_ftp"))
			    && iport!= 21)
	...
	if(r == 0)
	{
		...
		h = iptc_init("nat");
		if(h)
			r = delete_rule_and_commit(index, h, chain_name, "delete_redirect_rule");
	}
	return r;
}
...
delete_redirect_and_filter_rules(unsigned short eport, int proto)
{
	...
	else
	{
		...
		while (strcmp(nat_chain_name[++n], ""))
  		{
			r = __sub_delete_redirect_and_filter_rules(nat_chain_name[n], eport, proto, &iaddr, &iport);
		}
	}
...
#ifdef USE_MINIUPNP //EFM
		printf("\nports %d %d\n", (unsigned short)ntohl(mr->range[0].min.tcp.port),
		          (unsigned short)ntohl(mr->range[0].max.tcp.port));
#else

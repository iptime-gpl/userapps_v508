/* $Id: upnpsoap.c,v 1.1.1.1 2016/08/10 07:06:45 mt7623 Exp $ */
/* MiniUPnP project
...
GetExternalIPAddress(struct upnphttp * h, const char * action)
{
	 ...
	 * There is usually no NAT with IPv6 */
        ...
        if (use_ext_dynamic_ip) {
                ...
                get_wan_ip(wan_name, ext_ip_addr);

                if (!strcmp(ext_ip_addr, ""))
...
#ifdef USE_UPNP_RELAY
/*
 *          Upper Router                My Router              PC
 *           +--------+    UPNP Relay   +--------+            +----+
 *         eport     eport ----------- eport    iport ------ iport |
 *           +--------+                 +--------+            +----+
 */
        if (r == 0)
        {
                char msg[128];
                sprintf(msg, "upnp:forward,A,%s,%hu,%hu,%s", protocol, eport, eport, desc);
                signal_start(msg);
        }
#endif
...
#ifdef USE_UPNP_RELAY
                char msg[128];
                sprintf(msg, "upnp:forward,D,%s,%hu,0,null", protocol, eport);
                signal_start(msg);
#endif
		BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
	}

/* $Id: miniupnpd.c,v 1.3 2016/12/12 07:13:50 mt7623 Exp $ */
/* MiniUPnP project
...
extern char ROOTDEV_FRIENDLYNAME[128];
extern char ROOTDEV_MANUFACTURER[128];
extern char ROOTDEV_MANUFACTURERURL[128];
extern char ROOTDEV_MODELNAME[128];
extern char ROOTDEV_MODELDESCRIPTION[128];
extern char ROOTDEV_MODELURL[128];

extern char WANDEV_MANUFACTURERURL[128];
extern char WANDEV_MODELURL[128];
extern char WANCDEV_MODELURL[128];


extern int hwinfo_get_product_name(char *name);
extern int hwinfo_get_company_name(char *name);
extern int hwinfo_get_company_url(char *url);
extern int upnp_get_wan_name(char *wan_name);
extern int upnp_get_wan_ifname(char *ifname);
extern int create_pid(char *process_name);
extern int istatus_set_intvalue_direct(char *tag,int value);
#endif
...
#ifdef USE_MINIUPNP // EFM
static jmp_buf ext_if_name_env;
#endif
...
#ifdef USE_MINIUPNP //EFM
static void
sig_ext_if_name(int sig)
{
        upnp_get_wan_ifname(ext_if_name);
        syslog(LOG_NOTICE, "received signal %d, Ext Interface %s", sig, ext_if_name);

        siglongjmp(ext_if_name_env, -1);
}
#endif
...
#ifdef USE_MINIUPNP
static void get_rootDesc(void)
{
        char company_name[256], product_name[256], company_url[256];
        char buffer[256], *amp;
        ...
        get_si("pi",&pi);
        snprintf(product_name,256,pi.name);
        snprintf(buffer,256,pi.company);
        snprintf(company_url,256,pi.url);
#else
        hwinfo_get_product_name(product_name);
        hwinfo_get_company_name(buffer);
        hwinfo_get_company_url(company_url);
#endif
        if ((amp = strtok(buffer, "&")) != NULL)
        {
                sprintf(company_name, "%s", amp);
                while ((amp = strtok(NULL, "&")) != NULL)
                        sprintf(company_name, "%s&amp;%s", company_name, amp);
        }

        sprintf(ROOTDEV_FRIENDLYNAME, "%s %s", company_name, product_name);
        strcpy(ROOTDEV_MANUFACTURER, company_name);
        strcpy(ROOTDEV_MANUFACTURERURL, company_url);
        strcpy(ROOTDEV_MODELNAME, product_name);
#ifdef USE_IUX
        sprintf(ROOTDEV_MODELDESCRIPTION, "%s %s IUX", company_name, product_name);
#else
        sprintf(ROOTDEV_MODELDESCRIPTION, "%s %s NOIUX", company_name, product_name);
#endif
        strcpy(ROOTDEV_MODELURL, company_url);
        strcpy(WANDEV_MANUFACTURERURL, company_url);
        strcpy(WANDEV_MODELURL, company_url);
        strcpy(WANCDEV_MODELURL, company_url);
}
#endif
...
init(int argc, char * * argv, struct runtime_vars * v)
{
...
#ifdef USE_MINIUPNP //EFM
	 char local_ifname[20], local_ip[20], local_mask[20];
#endif
	...
	parselanaddr(lan_addr, local_ip);
	...
	else
	{
		for(i=0; i<num_options; i++)
		{
			...
			case UPNPEXT_IFNAME:
#ifdef USE_MINIUPNP //EFM
				strcpy(ext_if_name, ary_options[i].value);	
#else
			...
			case UPNPEXT_IP:
#ifdef USE_MINIUPNP //EFM
				strcpy(use_ext_ip_addr, ary_options[i].value);
#else
				use_ext_ip_addr = ary_options[i].value;
#endif
				break;
#ifdef USE_MINIUPNP //EFM
			case UPNPEXT_DYNAMIC_IP:
				use_ext_dynamic_ip = atoi(ary_options[i].value);
				break;
#endif
...
#ifdef USE_MINIUPNP //EFM
				// 2009.11.23, disable a function which unused rules are removed.
				v->clean_ruleset_interval = 0;
#else
			...
			case UPNPLEASEFILE:
#ifdef USE_MINIUPNP //EFM
				strcpy(lease_file, ary_options[i].value);
#else		
	...
	// openlog("miniupnpd", openlog_option, LOG_MINIUPNPD); // EFM
	...
	else
	{
                ...
                local_port=get_router_mgmt_port();
                if(local_port!=80)
                        snprintf(presentationurl, PRESENTATIONURL_MAX_LEN,
                         "http://%s:%d/", lan_addr[0].str,local_port);
                else
#endif
		snprintf(presentationurl, PRESENTATIONURL_MAX_LEN,
...
#ifdef USE_MINIUPNP // EFM
	if(signal(SIGUSR2, sig_ext_if_name) == SIG_ERR) {
		syslog(LOG_ERR, "Failed to ignore SIGHUP signals");
	}
#endif
...
#ifdef USE_MINIUPNP // EFM
static void  set_env(int on, int http_port)
{
        char local_ifname[32], buf[128];

        strcpy(local_ifname, IF_LOCAL);
        if (on)
        {
                system("/sbin/iptables -t filter -F upnp");
                system("/sbin/iptables -t filter -D INPUT -j upnp");

                sprintf(buf, "ifconfig %s allmulti", local_ifname);
                system(buf);
                sprintf(buf, "/sbin/iptables -t filter -I upnp -i "IF_LOCAL" -p tcp --dport %d -j ACCEPT", http_port);
                system(buf);
                sprintf(buf, "/sbin/iptables -t filter -I upnp ! -i "IF_LOCAL" -p tcp --dport %d -j DROP", http_port);
                system(buf);

                system("/sbin/iptables -t filter -I upnp -i "IF_LOCAL" -p udp --dport 1900 -j ACCEPT"); // SSDP_PORT (1900)
                system("/sbin/iptables -t filter -I upnp ! -i "IF_LOCAL" -p udp --dport 1900 -j DROP"); // SSDP_PORT (1900)

#ifdef USE_PORTFORWARD_V2
                system("/sbin/iptables -t filter -I upnp -d 127.0.0.1 -j ACCEPT"); // Local access
                istatus_set_intvalue_direct("upnp_http_port", http_port);
#endif
                system("/sbin/iptables -t filter -A INPUT -j upnp");
        ...
        else
        {
                ...
                system(buf);
                system("/sbin/iptables -t filter -F upnp");
                system("/sbin/iptables -t nat -F upnp");
                system("/sbin/iptables -t nat -F upnp_app_postroute");
#ifdef USE_PORTFORWARD_V2
                system("/sbin/iptables -t nat -F preroute_upnp");
#else
                system("/sbin/iptables -t nat -F upnp_in_public_preroute");
#endif
                system("/sbin/iptables -t filter -D INPUT -j upnp");
        }
}

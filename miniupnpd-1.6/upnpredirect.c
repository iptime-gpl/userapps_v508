/* $Id: upnpredirect.c,v 1.4 2017/03/14 09:14:30 hyrtlac Exp $ */
/* MiniUPnP project
               ...
               unsigned int timestamp)
{
	...
	fprintf(fd, "%s:%hu:%s:%hu:%u:%s\n",
	        ((proto==IPPROTO_TCP)?"TCP":"UDP"), eport, iaddr, iport, timestamp, desc);
#endif
	fclose(fd);
...
#else
/* str -> TCP;80  /  buf -> TCP;8000 */
#error "use ARCH_TIME : parse error"
		if (buf_size < str_size || strncmp(str, buf, str_size)!=0) {
			fwrite(buf, buf_size, 1, fdt);
		}
#endif
	...
	while(fgets(line, sizeof(line), fd)) {
		...
		*(desc++) = '\0';
		...
		if(r == -1) {
			...
			add_redirect_desc(eport, proto_atoi(proto), desc, timestamp);
		}
#endif
	}
	fclose(fd);

...
extern int rtl_8309_add_acl_for_dns(void);
extern int rtl_8309_remove_acl_for_dns(void);
#endif
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL_8309M_SUPPORT)
...
static const unsigned char iTunes_mc_addr[6] = {0x01, 0x00, 0x5e, 0x00, 0x00, 0xfb};
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info,int mCastFlag)
{
...
#endif
	{
...
#if defined(CONFIG_EFM_PATCH) && !defined(CONFIG_RTL_8367R_SUPPORT)
                        if((iptv_port == -1) && rtl_isWanPort(info->priv, vid, skb, pid, type))
                                goto relay_out;
		...
		{
#if defined(CONFIG_EFM_PATCH) && !defined(CONFIG_RTL_8367R_SUPPORT)
                        if((iptv_port == -1) && rtl_isWanDev(info->priv))
                                return 0;
...
#ifdef CONFIG_EFM_PATCH
                                   /* for local lan */
                                   && ((memcmp(&skb->data[0], iTunes_mc_addr, 6) != 0) && (memcmp(&skb->data[0], SSDP_mc_addr, 6) != 0))
#endif
				)
				{
...
#ifndef CONFIG_RTL_8367R_SUPPORT
#ifdef CONFIG_EFM_PATCH
        if(port0_port_config&0x80000000)
                return;
#endif
#endif
...
#if !(defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT) || defined(CONFIG_RTL_8309M_SUPPORT))
...
#elif defined(CONFIG_RTL_8309M_SUPPORT)
                                        {
                                        extern int32 rtk_stat_port_reset(uint32 port);
                                        uint32 phyport;
                                        for(phyport=0; phyport<9; phyport++)
	...
	for(i = 0; i < ETH_INTF_NUM; i++)
	{
		...
		if(_rtl86xx_dev.dev[i] && tmp_cp && tmp_cp->portmask && tmp_cp->opened) {
			...
			if(5 == portnum)
				continue;
#ifdef CONFIG_EFM_PATCH
                        if(((RTL_R32(PCRP0+portnum*4) & EnablePHYIf) == 0) && !diag_status)
#else
...
#if defined(CONFIG_EFM_PATCH) && defined(CONFIG_RTL_8309M_SUPPORT)
typedef enum rtk_igmp_type_e
{
    IGMP_IPV4 = 0,
    IGMP_PPPOE_IPV4,
...
unsigned int fdbmanage_port = -1;

static int32 fdbmanage_read(struct seq_file *s, void *v)
{
        if(fdbmanage_port != -1)
                seq_printf(s, "%d\n", fdbmanage_port);
        else
...
static int32 fdbmanage_write(struct file *fp, const char *buff, unsigned long len, void *data )
{
        char            tmpbuf[128];
        ...
        if (buff && !copy_from_user(tmpbuf, buff, len)) {
                ...
                if (!memcmp(cmd_addr, "madd", 4))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[0] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[1] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[2] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[3] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[4] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[5] = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        port_map.bits[0] = (uint32)simple_strtoul(tokptr, NULL, 0);

                        val = rtk_l2_mcastAddr_add(&macaddr, 0, port_map);
                        printk("[Multicast Addr ADD] [RESULT %d] [%02X:%02X:%02X:%02X:%02X:%02X] [PORTMAP %x]\n"
                        ,val,macaddr.octet[0],macaddr.octet[1],macaddr.octet[2],macaddr.octet[3],macaddr.octet[4],macaddr.octet[5], port_map.bits[0]);
                }
                else if (!memcmp(cmd_addr, "mdel", 4))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[0] = (uint8)simple_strtoul(tokptr, NULL, 0);
                        ...
                        val = rtk_l2_mcastAddr_del(&macaddr, 0);
                        printk("[Multicast Addr DEL] [RESULT %d] [%02X:%02X:%02X:%02X:%02X:%02X]\n"
                        ,val,macaddr.octet[0],macaddr.octet[1],macaddr.octet[2],macaddr.octet[3],macaddr.octet[4],macaddr.octet[5]);
                }
		else if (!memcmp(cmd_addr, "mget", 4))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[0] = (uint8)simple_strtoul(tokptr, NULL, 0);
                        ...
                        val = rtk_l2_mcastAddr_get(&macaddr, 0, &port_map);
                        printk("[Multicast Addr GET] [RESULT %d] [%02X:%02X:%02X:%02X:%02X:%02X] [PORTMAP %x]\n"
                        ,val,macaddr.octet[0],macaddr.octet[1],macaddr.octet[2],macaddr.octet[3],macaddr.octet[4],macaddr.octet[5], port_map.bits[0]);
                }
                else if (!memcmp(cmd_addr, "uget", 4))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        macaddr.octet[0] = (uint8)simple_strtoul(tokptr, NULL, 0);
                        ...
                        memset(&L2_data, 0x0, sizeof(rtk_l2_ucastAddr_t));
                        if((val = rtk_l2_addr_get(&macaddr, 0, &L2_data)) != 0)
                        {
                                printk("[Unicast Addr GET] [RESULT %d] [%02X:%02X:%02X:%02X:%02X:%02X] [L2Port %d]\n"
                                ,val,macaddr.octet[0],macaddr.octet[1],macaddr.octet[2],macaddr.octet[3],macaddr.octet[4],macaddr.octet[5],L2_data.port);
                                fdbmanage_port = -1;
                                goto errout;
                        }
                        fdbmanage_port = L2_data.port;
                }
                else if (!memcmp(cmd_addr, "unknown_set", 11))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        val = (uint8)simple_strtoul(tokptr, NULL, 0);

                        tokptr = strsep(&strptr," ");
                        if (tokptr && (!memcmp(tokptr, "fwd", 3) || !memcmp(tokptr, "forward", 7))){
                                printk("[Trap Unknown Multicast action SET] [port %d][forward]\n",val);
                                rtk_trap_unknownMcastPktAction_set(val, MCAST_IPV4, MCAST_ACTION_FORWARD);
                        }else{
                                printk("[Trap Unknown Multicast action SET] [port %d][drop]\n", val);
                                rtk_trap_unknownMcastPktAction_set(val, MCAST_IPV4, MCAST_ACTION_DROP);
                        }
                }
		else if (!memcmp(cmd_addr, "unknown_get", 11))
                {
                        rtk_trap_mcast_action_t __act;
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        val = (uint8)simple_strtoul(tokptr, NULL, 0);

                        rtk_trap_unknownMcastPktAction_get(val, MCAST_IPV4, &__act);
                        printk("[Trap Unknown Multicast action GET] [port %d][%s]\n", val, ((__act == MCAST_ACTION_FORWARD)?"forward":"drop"));
                }
                else if (!memcmp(cmd_addr, "igmpsnooping_set", 16))
                {
                        val = rtl8309n_igmp_enable_set(RTL8309N_IGMP, 1);
                        printk("[IGMPSNOOPING SET to ENABLE] [RESULT %d]\n", val);
                }
                else if (!memcmp(cmd_addr, "igmpsnooping_get", 16))
                {
                        uint32 enabled;
                        val = rtl8309n_igmp_enable_get(RTL8309N_IGMP, &enabled);
                        printk("[IGMPSNOOPING GET] [RESULT %d] [%s]\n", val, ((enabled)?"ENABLED":"DISABLED"));
                }
                else if (!memcmp(cmd_addr, "trap_set", 8))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL){goto errout;}
                        val = (uint8)simple_strtoul(tokptr, NULL, 0);
                        ...
                        tokptr = strsep(&strptr," ");
                        if (tokptr && (!memcmp(tokptr, "fwd", 3) || !memcmp(tokptr, "forward", 7))){
                                printk("[Trap IGMP Control packet action SET] [forward]\n");
                                rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_FORWARD);
                        }else if (tokptr && (!memcmp(tokptr, "cpycpu", 6))){
                                printk("[Trap IGMP Control packet action SET] [copy to cpu]\n");
                                rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_COPY2CPU);
                        }else if (tokptr && (!memcmp(tokptr, "trpcpu", 6))){
                                printk("[Trap IGMP Control packet action SET] [trap to cpu]\n");
                                rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_TRAP2CPU);
                        }else if (tokptr && (!memcmp(tokptr, "drop", 4))){
                                printk("[Trap IGMP Control packet action SET] [drop]\n");
                                rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_DROP);
                        }else{
                                printk("[Trap IGMP Control packet action SET] [forward]\n");
                                rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_FORWARD);
                        }
                }
                else if (!memcmp(cmd_addr, "trap_get", 8))
                {
                        rtk_trap_igmp_action_t __act;

                        rtk_trap_igmpCtrlPktAction_get(IGMP_IPV4, &__act);

                        switch(__act)
                        {
                                case IGMP_ACTION_FORWARD:
                                        printk("[Trap IGMP Control packet action GET] [forward]\n");    break;
                                case IGMP_ACTION_COPY2CPU:
                                        printk("[Trap IGMP Control packet action GET] [copy to cpu]\n");        break;
                                case IGMP_ACTION_TRAP2CPU:
                                        printk("[Trap IGMP Control packet action GET] [trap to cpu]\n");        break;
                                case IGMP_ACTION_DROP:
                                        printk("[Trap IGMP Control packet action GET] [drop]\n");       break;
                                default:
                                        printk("[Trap IGMP Control packet action GET] [error]\n");      break;
                        }
                }
                else
                {
        ...
        return len;
errout:
        printk("[FDBMANAGE INPUT ERROR]\n");
        printk("Usage -------------------  [] : required, () : optional\n");
        printk("[Multicast ADD] : madd [m1] [m2] [m3] [m4] [m5] [m6] [port map]\n");
        printk("[Multicast DEL] : mdel [m1] [m2] [m3] [m4] [m5] [m6]\n");
        printk("[Multicast GET] : mget [m1] [m2] [m3] [m4] [m5] [m6]\n");
        printk("[Unicast GET] : uget [m1] [m2] [m3] [m4] [m5] [m6]\n");
        printk("[Trap Unknown Multicast action SET] : unknown_set [port] (fwd || forward || drop)\n");
        printk("[Trap Unknown Multicast action GET] : unknown_get [port]\n");
        printk("[Trap IGMP Control packet action SET] : trap_set [fwd || forward || cpycpu || trpcpu || drop]\n");
        printk("[Trap IGMP Control packet action GET] : trap_get\n");
        printk("[IGMP Snooping SET to ENABLE] : igmpsnooping_set\n");
        printk("[IGMP Snooping GET current status] : igmpsnooping_get\n");
        return len;
}
int fdbmanage_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, fdbmanage_read, NULL));
}
static ssize_t fdbmanage_single_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
            return fdbmanage_write(file, userbuf,count, off);
}
struct file_operations fdbmanage_proc_fops= {
        .open           = fdbmanage_single_open,
        .write          = fdbmanage_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
unsigned int debug_reg_addr = 0;
unsigned int debug_reg_val = 0;

static int32 proc_write_debug_reg( struct file *filp, const char *buff,unsigned long len, void *data )
{
        char            tmpbuf[256];
        char *ptr, *sepptr;
        ...
        if(!buff)
                return len;
        if (copy_from_user(tmpbuf, buff, len))
                return len;
        tmpbuf[len-1] = '\0';
        ...
        ptr = strsep(&sepptr, " ");
        if(!ptr)        return len;
        debug_reg_addr = simple_strtoul(ptr, NULL, 16);
        ptr = strsep(&sepptr, " ");
        if(!ptr){debug_reg_val = 0;     return len;}
        debug_reg_val = simple_strtoul(ptr, NULL, 16);

        if(debug_reg_addr){
                WRITE_MEM32(debug_reg_addr, debug_reg_val);
        }
        else{
                debug_reg_addr = 0;
                debug_reg_val = 0;
...
static int32 proc_write_iptv_port( struct file *filp, const char *buff,unsigned long len, void *data )
{
...
#if     0
        if(iptv_port != -1)
                rtl865x_delVlanPortMember(RTL_LANVLANID & 4095, (0x1<<iptv_port));
#endif

        iptv_port = simple_strtol(tmpbuf, NULL, 0);

        if((iptv_port & EXTERNAL_SWITCH_PORT_FLAG) == EXTERNAL_SWITCH_PORT_FLAG)
                iptv_port &= ~(EXTERNAL_SWITCH_PORT_FLAG);

        rtl865x_delVlanPortMember(RTL_LANVLANID & 4095, (0x1<<iptv_port));

        rtl865x_addVlanPortMember(RTL_WANVLANID & 4095, (0x1<<iptv_port));
        rtl8651_setAsicPvid(iptv_port,     RTL_WANVLANID);
        cp1 = NETDRV_PRIV(_rtl86xx_dev.dev[0]);
        cp2 = NETDRV_PRIV(_rtl86xx_dev.dev[1]);
        cp1->portmask = rtl865x_getVlanPortMask(RTL_LANVLANID);
        cp2->portmask = rtl865x_getVlanPortMask(RTL_WANVLANID);

#ifdef CONFIG_RTL_8370_SUPPORT_XXXX
        rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_FORWARD);
        rtk_trap_unknownMcastPktAction_set(RTL8370_WAN, IGMP_IPV4, IGMP_ACTION_FORWARD);
#endif

        printk("LAN portmask : %08x WAN portmask: %08x\n" , cp1->portmask , cp2->portmask );
...
#ifdef CONFIG_RTL_8367R_SUPPORT
        {
        unsigned int PriDec[8] = {0x01,0x40,0x80,0x02,0x10,0x20,0x08,0x04};
        unsigned int Pri2qid[8] = {0, 0,1,1,2,3,7,7};
        unsigned int Qweights[8] = {0,0,0,0,0,0,0,0};
        int dscp, port;
        ...
        for (dscp=0; dscp<=7; dscp++)   rtk_qos_dscpPriRemap_set(dscp, 0);
        for (dscp=8; dscp<=15; dscp++)  rtk_qos_dscpPriRemap_set(dscp, 1);
        for (dscp=16; dscp<=23; dscp++) rtk_qos_dscpPriRemap_set(dscp, 2);
        for (dscp=24; dscp<=31; dscp++) rtk_qos_dscpPriRemap_set(dscp, 3);
        for (dscp=32; dscp<=39; dscp++) rtk_qos_dscpPriRemap_set(dscp, 4);
        for (dscp=40; dscp<=47; dscp++) rtk_qos_dscpPriRemap_set(dscp, 5);
        for (dscp=48; dscp<=55; dscp++) rtk_qos_dscpPriRemap_set(dscp, 6);
        for (dscp=56; dscp<=63; dscp++) rtk_qos_dscpPriRemap_set(dscp, 7);
...
#define WAN_PORT 0
static int32 proc_write_wan_port_type( struct file *filp, const char *buff,unsigned long len, void *data )
{
        struct dev_priv *cp1, *cp2;
        char            tmpbuf[256];
        ...
        cp2 = NETDRV_PRIV(_rtl86xx_dev.dev[1]);
        tmpbuf[len-1] = '\0';
        rtlglue_printf("Before LAN portmask : %08x WAN portmask: %08x\n" , cp1->portmask , cp2->portmask );

        if(!strcmp(tmpbuf,"wan"))
        {
                rtlglue_printf("WAN PORT TYPE: WAN\n");
                if(rtl865x_getVlanPortMask(RTL_WANVLANID)==0)
                {
                        rtlglue_printf("ADD VLAN for WAN first\n");
                        rtl865x_addVlan(RTL_WANVLANID);
                }
                rtl865x_delVlanPortMember(RTL_LANVLANID & 4095, (0x1<<WAN_PORT));
                rtl865x_addVlanPortMember(RTL_WANVLANID & 4095, (0x1<<WAN_PORT));
                rtl8651_setAsicPvid(WAN_PORT, RTL_WANVLANID);
                cp1->portmask = rtl865x_getVlanPortMask(RTL_LANVLANID);
                cp2->portmask = rtl865x_getVlanPortMask(RTL_WANVLANID);
        }
        else
        {
                rtlglue_printf("WAN PORT TYPE: LAN\n");
                rtl865x_delVlanPortMember(RTL_WANVLANID & 4095, (0x1<<WAN_PORT));
                rtl865x_addVlanPortMember(RTL_LANVLANID & 4095, (0x1<<WAN_PORT));
                rtl8651_setAsicPvid(WAN_PORT, RTL_LANVLANID);
                cp1->portmask = rtl865x_getVlanPortMask(RTL_LANVLANID);
                cp2->portmask = rtl865x_getVlanPortMask(RTL_WANVLANID);
        }

        rtlglue_printf("After LAN portmask : %08x WAN portmask: %08x\n" , cp1->portmask , cp2->portmask );

        return len;
}

static int efm_common_read_proc(struct seq_file *s, void *v)
{
        return 0;
}

int iptv_port_entry_open(struct inode *inode, struct file *file)
{
        return(single_open(file, efm_common_read_proc, NULL));
}

static ssize_t iptv_port_entry_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return proc_write_iptv_port(file, userbuf,count, off);
}

int wan_port_type_entry_open(struct inode *inode, struct file *file)
{
        return(single_open(file, efm_common_read_proc, NULL));
}

static ssize_t wan_port_type_entry_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return proc_write_wan_port_type(file, userbuf,count, off);
}

static int debug_reg_read_proc(struct seq_file *s, void *v)
{
        if(debug_reg_addr)
                seq_printf(s,"[ADDR]%x [(READ)VALUE]%x [(applied)VALUE]%x\n"
                        , debug_reg_addr, READ_MEM32(debug_reg_addr), debug_reg_val);
        else
                seq_printf(s,"DEBUG REGISER NO APPLIED\necho \"[ADDR] [VALUE]\" > /proc/debug_reg\n[SWCORE_BASE]%x\n[MIB_COUNTER_BASE]%x\n"
                        ,SWCORE_BASE, MIB_COUNTER_BASE);
        return 0;
}

int debug_reg_open(struct inode *inode, struct file *file)
{
        return(single_open(file, debug_reg_read_proc, NULL));
}

static ssize_t debug_reg_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return proc_write_debug_reg(file, userbuf,count, off);
}

struct file_operations iptv_port_entry = {
        .open            = iptv_port_entry_open,
        .write           = iptv_port_entry_write,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release         = single_release,
};

struct file_operations wan_port_type_entry = {
        .open            = wan_port_type_entry_open,
        .write           = wan_port_type_entry_write,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release         = single_release,
};

struct file_operations debug_reg_entry = {
        .open            = debug_reg_open,
 ...
 int rtl819x_get_port_status(int portnum , struct lan_port_status *port_status)
{
		...
		uint32	data0;
...
#endif
                {
                        rtk_port_phyStatus_get(portnum, &link, &speed, &duplex);
                        port_status->link = (unsigned char)link;
                        port_status->speed = (unsigned char)speed;
                        port_status->duplex = (unsigned char)duplex;
                }
#ifdef CONFIG_RTL_8370_SUPPORT
                else if(portnum == 8)
                {
                        rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 1, &regData);
                        rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 1, &regData);
                        port_status->link = (regData & (1 << 2)) ? 1 : 0;
                        port_status->speed = 0;
                        port_status->duplex = 0;
                        if (port_status->link)
                        {
                                rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 10, &regData);
                                rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 10, &regData);
                                ...
                                {
                                        rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 5, &regData);
                                        rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 5, &regData);
                                        if (regData & (1<<8))
                                        {
                                                port_status->speed = 1;
                ...
                if((portnum & EXTERNAL_SWITCH_PORT_FLAG) == EXTERNAL_SWITCH_PORT_FLAG)
                {
                        uint32 link, speed, duplex;
                        portnum &= ~(EXTERNAL_SWITCH_PORT_FLAG);
                ...
                else
                {
                        ...
                        switch(data0)
                        {
                                case PortStatusLinkSpeed1000M:  port_status->speed = PORT_SPEED_1000M;  break;
                                case PortStatusLinkSpeed100M:   port_status->speed = PORT_SPEED_100M;   break;
                                case PortStatusLinkSpeed10M:    port_status->speed = PORT_SPEED_10M;    break;
                                default:                        port_status->speed = 0;                 break;
                        }
                }
                ...
                if (portnum >= 5 && portnum <= 8)
                {
                        rtk_port_mac_ability_t Portstatus;

                        rtk_port_macStatus_get((unsigned int)(portnum-5), &Portstatus);
                        port_status->link = Portstatus.link;
                        port_status->speed = Portstatus.speed;
                        port_status->duplex = Portstatus.duplex;
                        port_status->nway = Portstatus.nway;
 ...
 int rtl819x_get_port_stats(int portnum , struct port_statistics *port_stats)
 {
	...
	uint32 addrOffset_fromP0 =0;
...
#ifdef CONFIG_EFM_PATCH
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)
        rtk_stat_port_cntr_t  dev_port_stat;
...
#endif
        {
                rtk_stat_port_getAll(portnum, &dev_port_stat);

                port_stats->rx_bytes = dev_port_stat.ifInOctets;
                port_stats->rx_unipkts = dev_port_stat.ifInUcastPkts;
#ifdef CONFIG_RTL_8367R_SUPPORT
                port_stats->rx_mulpkts = dev_port_stat.ifInMulticastPkts;
                port_stats->rx_bropkts = dev_port_stat.ifInBroadcastPkts;
#else
                port_stats->rx_mulpkts = dev_port_stat.etherStatsMcastPkts;
                port_stats->rx_bropkts = dev_port_stat.etherStatsBcastPkts;
#endif
                port_stats->rx_discard = dev_port_stat.etherStatsDropEvents;
                port_stats->rx_error =  dev_port_stat.dot3StatsFCSErrors + dev_port_stat.dot3StatsSymbolErrors + dev_port_stat.etherStatsJabbers + dev_port_stat.etherStatsFragments ;

                port_stats->tx_bytes = dev_port_stat.ifOutOctets;
                port_stats->tx_unipkts = dev_port_stat.ifOutUcastPkts;
                port_stats->tx_mulpkts = dev_port_stat.ifOutMulticastPkts;
                port_stats->tx_bropkts = dev_port_stat.ifOutBrocastPkts;
                port_stats->tx_discard = dev_port_stat.dot1dBasePortDelayExceededDiscards;
                port_stats->tx_error = dev_port_stat.etherStatsCollisions;
        }
        return 0;
#endif
        ...
        {
                uint32 enable;
                rtk_mib_counter_t counter;
                int32 ret;
                unsigned long long totalbyte = 0;

                portnum &= ~(EXTERNAL_SWITCH_PORT_FLAG);

                rtl8309n_mib_enable_get(&enable);
                if (enable == 0){
                        rtl8309n_mib_enable_set(1);
                        rtl8309n_mib_start(portnum);
                        rtl8309n_mib_stop(portnum);
                }

                memset(port_cntrs, 0, 4*sizeof(uint32));
                for (counter=0; counter<15; counter++){
                        ret = rtk_mib_get(portnum, counter, port_cntrs);

                        if (ret == 0)
                        {
                                switch(counter) {
                                        case MIB_TXBYTECNT:
                                                totalbyte = ((port_cntrs[3] & 0xFFFF)<<16) | (port_cntrs[2] & 0xFFFF);
                                                totalbyte = (totalbyte << 32) | ((port_cntrs[1] & 0xFFFF)<<16) | (port_cntrs[0] & 0xFFFF);
                                                port_stats->tx_bytes = totalbyte;
                                                break;
                                        case MIB_RXBYTECNT:
                                                totalbyte = ((port_cntrs[3] & 0xFFFF)<<16) | (port_cntrs[2] & 0xFFFF);
                                                totalbyte = (totalbyte << 32) | ((port_cntrs[1] & 0xFFFF)<<16) | (port_cntrs[0] & 0xFFFF);
                                                port_stats->rx_bytes = totalbyte;
                                                break;
                                        case MIB_TXPKTCNT:      break;
                                        case MIB_RXPKTCNT:      break;
                                        case MIB_RXDPCNT:
                                                port_stats->rx_discard = port_cntrs[0];
                                                break;
                                        case MIB_RXCRCCNT:
                                                port_stats->rx_error = port_cntrs[0];
                                                break;
                                        case MIB_RXFRAGCNT:     break;
                                        case MIB_TXBRDCNT:
                                                port_stats->tx_bropkts = port_cntrs[0];
                                                break;
                                        case MIB_RXBRDCNT:
                                                port_stats->rx_bropkts = port_cntrs[0];
                                                break;
                                        case MIB_TXMULTCNT:
                                                port_stats->tx_mulpkts = port_cntrs[0];
                                                break;
                                        case MIB_RXMULTCNT:
                                                port_stats->rx_mulpkts = port_cntrs[0];
                                                break;
                                        case MIB_TXUNICNT:
                                                port_stats->tx_unipkts = port_cntrs[0];
                                                break;
                                        case MIB_RXUNICNT:
                                                port_stats->rx_unipkts = port_cntrs[0];
                                                break;
                                        case MIB_RXSYMBLCNT:    break;
                                        default:                break;
                                }
                        }
                        else
                                printk("switch mib get fail!\n");
                }
                /* not supported */
                port_stats->tx_error = 0;
        }
        else
        {
		addrOffset_fromP0 = (portnum) * MIB_ADDROFFSETBYPORT;
        ...
        if (portnum >= 5 && portnum <= 8)
        {
                unsigned int iport = (unsigned int)(portnum - 5);

                rtk_stat_port_get(iport, STAT_IfInOctets, &port_stats->rx_bytes);
                port_stats->rx_unipkts = rtk_get_port_stats(iport, STAT_IfInUcastPkts);
                port_stats->rx_mulpkts = rtk_get_port_stats(iport, STAT_EtherStatsMulticastPkts);
                port_stats->rx_bropkts = rtk_get_port_stats(iport, STAT_EtherStatsBroadcastPkts);
                port_stats->rx_discard = rtk_get_port_stats(iport, STAT_Dot1dTpPortInDiscards);
                port_stats->rx_error = rtk_get_port_stats(iport, STAT_Dot3StatsFCSErrors);

                rtk_stat_port_get(iport, STAT_IfOutOctets, &port_stats->tx_bytes);
                port_stats->tx_unipkts = rtk_get_port_stats(iport, STAT_IfInUcastPkts);
                port_stats->tx_mulpkts = rtk_get_port_stats(iport, STAT_IfOutMulticastPkts);
                port_stats->tx_bropkts = rtk_get_port_stats(iport, STAT_IfOutBroadcastPkts);
                port_stats->tx_discard = rtk_get_port_stats(iport, STAT_Dot1dBasePortDelayExceededDiscards);
                port_stats->tx_error = rtk_get_port_stats(iport, STAT_EtherStatsCollisions);
...
#if defined CONFIG_EFM_PATCH_FOR_MULTI || defined CONFIG_EFM_PATCH_MOBILE_AP
#define EXTEND_GPIO_NUM 10
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
#define LANWAN_GPIO_NUM 14
#else
#define LANWAN_GPIO_NUM 11
#endif

#define RTL_WANPORT_MASK_FOR_MULTI                0x10
#define RTL_LANPORT_MASK_FOR_MULTI                0x10f
extern int check_bt(int gpio);
...
int adjust_vlan_by_lanwan_switch( void )
{
rtlglue_printf("==========> Configuring Port to ");

        /* WRITE_MEM32(PIN_MUX_SEL_2, 0x1b); */
        WRITE_MEM32(PIN_MUX_SEL_2, 0xffff);

        RTL_W32(PABCD_CNR, (RTL_R32(PABCD_CNR) & (~(0x1<<LANWAN_GPIO_NUM))));
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) & (~(0x1<<LANWAN_GPIO_NUM))));

#ifdef CONFIG_EFM_PATCH_FOR_MULTI
        RTL_W32(PABCD_CNR, (RTL_R32(PABCD_CNR) & (~(0x1<<10))));
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) & (~(0x1<<10))));
#endif

#ifdef CONFIG_EFM_PATCH_FOR_MULTI
        if(!check_bt(LANWAN_GPIO_NUM)
                && check_bt(EXTEND_GPIO_NUM)
        )
        {
                rtlglue_printf("WAN\n");
                rtl865x_wanPortMask=RTL_WANPORT_MASK_FOR_MULTI;
                rtl865x_lanPortMask=RTL_LANPORT_MASK_FOR_MULTI;
        }
        else
                rtlglue_printf("LAN\n");
#endif
        ...
        {
                rtlglue_printf("LAN\n");
                rtl865x_wanPortMask=0x0;
                rtl865x_lanPortMask=0x11f;
        }
#endif
	...
	FullAndSemiReset();
	{
		...
		*/
		...
		if (RTL865X_PORTMASK_UNASIGNED==rtl865x_wanPortMask)
		{
			...
			#if !defined(CONFIG_RTL_8367R_SUPPORT) && !defined(CONFIG_RTL_8211F_SUPPORT) && !defined(CONFIG_RTL_8309M_SUPPORT) && !defined(CONFIG_RTL_83XX_SUPPORT)
			{
...
//	rtlglue_printf("	creating eth0~eth%d...\n",totalVlans-1 );
	...
	for(i=0;i<totalVlans;i++)
	{
		if (TRUE==vlanconfig[i].isWan)
		{
#if defined(CONFIG_EFM_PATCH) && defined(CONFIG_EFM_KT_3WAN_PORTS)
                        mCastConfig.externalPortMask = 0x3;
#else
			mCastConfig.externalPortMask |=vlanconfig[i].memPort;
#endif
		}
...
#ifdef CONFIG_EFM_PATCH
        proc_create_data("iptv_port",0,&proc_root,&iptv_port_entry,NULL);
        proc_create_data("wan_port_type",0,&proc_root,&wan_port_type_entry,NULL);
        proc_create_data("debug_reg",0,&proc_root,&debug_reg_entry,NULL);
#ifdef CONFIG_RTL_8309M_SUPPORT
        proc_create_data("fdbmanage",0,&proc_root,&fdbmanage_proc_fops,NULL);
#endif
#endif	
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT) || defined(CONFIG_RTL_8309M_SUPPORT)
...
#if defined(CONFIG_RTL_8309M_SUPPORT)
                else if (!memcmp(cmd_addr, "8309port", 4))
                {
                        port=(uint32)simple_strtol(tokptr, NULL, 0);
                        if (port > 5)
                                goto errout;
                        ...
                        panic_printk("successfully set 8309port %d to %s", port, tokptr);
                }
#endif

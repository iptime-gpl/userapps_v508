...
extern int32 rtk_mib_get(uint32 port, rtk_mib_counter_t counter, uint32 *pValue);
...
#if defined(CONFIG_EFM_PATCH) && defined(CONFIG_RTL_8309M_SUPPORT)
                        if(port_mask == 0)      rtl8309n_mirror_portBased_set(port_mask, rx_mask, tx_mask);
                        for(i = 0 ; port_mask > 0 ; i++){
                                if((port_mask & 1) == 1)
                                        rtl8309n_mirror_portBased_set(i, rx_mask, tx_mask);
...
static int32 port_status_read(struct seq_file *s, void *v)
{
	...
	for(port=PHY0;port<=PHY4;port++)
	{
        ...
        seq_printf(s, "\n");
        seq_printf(s, "Dump 97F Port Status:\n");
        for(port=PHY0;port<=CPU;port++)
        {
...
{
	char		tmpbuf[64];
	uint32  port_mask, extports = 0, intports = 0;
	char		*strptr, *cmd_addr;
	char		*tokptr, *bkptr;
	int 	type;
	int 		port;
#if !defined(CONFIG_RTL_8367R_SUPPORT) && !defined(CONFIG_RTL_8370_SUPPORT) && !defined(CONFIG_RTL_8309M_SUPPORT) && !defined(CONFIG_RTL_83XX_SUPPORT)
	int forceMode = 0;
	int forceLink = 0;
...
#define SPEED1000M 	2
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		/* port internal_mask external_mask type */
                /* port mask type */
		...
		if (!memcmp(cmd_addr, "port", 4))
		{
			...
			bkptr = strsep(&strptr," ");
                        ...
                        {
                                intports = port_mask;
                                extports = simple_strtol(tokptr, NULL, 0);
                                tokptr = bkptr;
                        }
...
extern uint32  port_cntrs[4];

void display_8309m_port_stat(uint32 port, int32 counter, uint32 *pPort_cntrs)
{
        ...
        switch(counter) {
                case MIB_TXBYTECNT:
                        totalbyte |= pPort_cntrs[1];
                        totalbyte = (totalbyte<<32) | (pPort_cntrs[0]);
                        rtlglue_printf("   TXBYTECNT  %llu\n", totalbyte);
                        break;
                case MIB_RXBYTECNT:
                        totalbyte |= pPort_cntrs[1];
                        totalbyte = (totalbyte<<32) | (pPort_cntrs[0]);
                        rtlglue_printf("   RXBYTECNT  %llu\n", totalbyte);
                        break;
                case MIB_TXPKTCNT:
                        rtlglue_printf("   TXPKTCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXPKTCNT:
                        rtlglue_printf("   RXPKTCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXDPCNT:
                        rtlglue_printf("   RXDPCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXCRCCNT:
                        rtlglue_printf("   RXCRCCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXFRAGCNT:
                        rtlglue_printf("   RXFRAGCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_TXBRDCNT:
                        rtlglue_printf("   TXBRDCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXBRDCNT:
                        rtlglue_printf("   RXBRDCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_TXMULTCNT:
                        rtlglue_printf("   TXMULTCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXMULTCNT:
                        rtlglue_printf("   RXMULTCNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_TXUNICNT:
                        rtlglue_printf("   TXUNICNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXUNICNT:
                        rtlglue_printf("   RXUNICNT  %u\n", pPort_cntrs[0]);
                        break;
                case MIB_RXSYMBLCNT:
                        rtlglue_printf("   RXSYMBLCNT  %u\n", pPort_cntrs[0]);
                        break;
        }
}
#endif
...
#endif
#if defined(CONFIG_RTL_8309M_SUPPORT)
                        if(strncmp(cmdptr, "83xx",4) == 0) {
                                uint32 enable;
                                for (portNum=0; portNum<=SWITCH_MAX_PORT_NUM; portNum++) {
                                        ...
                                        for (counter=0; counter<15; counter++){
                                                ...
                                                if (ret == 0)
                                                        display_8309m_port_stat(portNum, counter,port_cntrs);
                                                else
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		else if (!memcmp(cmd_addr, "8325phyw", 8))
		{
			...
			else
			{
...
#endif
#if defined(CONFIG_RTL_8309M_SUPPORT)
                else if (!memcmp(cmd_addr, "83xxphyr", 8))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL)
                        {
                        ...
                        ret = rtl8309n_reg_get(phyId, regId, pageId, &regData);
                        if(ret==SUCCESS)
                        {
                                rtlglue_printf("read 83xx phyId(%d), regId(%d), pageId(%d), regData:0x%x\n", phyId, regId, pageId, regData);
                        }
                        else
                        {
                                rtlglue_printf("error input!\n");
                ...
                else if (!memcmp(cmd_addr, "83xxphyw", 8))
                {
                        tokptr = strsep(&strptr," ");
                        if (tokptr==NULL)
                        {
                        ...
                        ret = rtl8309n_reg_set(phyId, regId, pageId, regData);
                        if(ret==SUCCESS)
                        {
                                rtlglue_printf("Write 83xx phyId(%d), regId(%d), pageId(%d), regData:0x%x\n", phyId, regId, pageId, regData);
                        }
                        else
                        {
                                rtlglue_printf("error input!\n");
                ...
                else if (!memcmp(cmd_addr, "83xxinit", 8))
                {
                        init_8309m();
                }
#endif
...
static int32 proc_led_read(struct seq_file *s, void *v)
{
        return 0;
}
...
static int32 proc_led_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
        ...
        if (buff && !copy_from_user(tmpbuf, buff, len))
        {
                ...
                rtlglue_printf("on/off");
        return len;
}

int fc_efmled_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, proc_led_read, NULL));
}
static ssize_t fc_efmled_single_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
            return proc_led_write(file, userbuf,count, off);
}
struct file_operations fc_efmled_proc_fops= {
        .open           = fc_efmled_single_open,
        .write          = fc_efmled_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#endif
#define GPIO_PORTS_LED_MASK 0x7c00
#define MUX2_PORTS_GPIO_VALUE (0x3 | (0x3<<3) | (0x3<<6) | (0x3<<9) | (0x3<<12))
int set_port_led(int flag)
{
        if(flag)
        {
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL8367R_VB_API)
                rtk_set_portled_all(1);
#ifdef CONFIG_RTL8367R_VB_API
                WRITE_MEM32(PIN_MUX_SEL_2, 0x0);
#endif
...
#ifdef CONFIG_RTL_8309M_SUPPORT
                /* 8309M LED ON code  + MUX RTL8197F WAN PORT LED control */
                int recov = READ_MEM32(PIN_MUX_SEL14);
                recov &= ~(0xF << 28);  /* WAN LED */
                WRITE_MEM32(PIN_MUX_SEL14, recov);
                rtk_set_portled_all(1); /* SWITCH(0~7 LAN) LED OFF */
#endif
...
#ifdef CONFIG_RTL_8367R_SUPPORT
                printk("set_port_led:TBD\n");

#else
                printk("set_port_led : %08x \n",  REG32(PIN_MUX_SEL));
                REG32(PIN_MUX_SEL) &= ~0xFFF;
#endif
#endif
        ...
        {
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL8367R_VB_API)
                rtk_set_portled_all(0);
#ifdef CONFIG_RTL8367R_VB_API
                WRITE_MEM32(PIN_MUX_SEL_2, 0xffffffff);
#endif
#else
...
#ifdef CONFIG_RTL_8309M_SUPPORT
                /* 8309M LED OFF code  + MUX RTL8197F WAN PORT LED control */
                int recov = READ_MEM32(PIN_MUX_SEL14);
                recov |= (0x2 << 28);   /* WAN LED */
                WRITE_MEM32(PIN_MUX_SEL14, recov);
                recov = READ_MEM32(PEFGH_DIR);
                WRITE_MEM32(PEFGH_DIR, (recov | (1 << 26)));
                recov = READ_MEM32(PEFGH_DAT);
                WRITE_MEM32(PEFGH_DAT, (recov | (1 << 26)));
                rtk_set_portled_all(0); /* SWITCH(0~7 LAN) LED OFF */
#endif
	...
	if(rtl865x_proc_dir)
	{
		...
		/*FC threshold*/
		{
			...
			else
			{
                ...
                {
#ifdef CONFIG_RTL_PROC_NEW
            proc_create_data("led",0,rtl865x_proc_dir,&fc_efmled_proc_fops,NULL);
#else
...
#endif
#ifdef SERIAL_DUMP_DEBUG
        {
                static struct proc_dir_entry *sdump_entry;
                static struct proc_dir_entry *serialstat_entry;
                sdump_entry = create_proc_entry("serialdump",0,rtl865x_proc_dir);
                if(sdump_entry != NULL)
                        sdump_entry->read_proc = sdump_read;
                serialstat_entry = create_proc_entry("serialstat",0,rtl865x_proc_dir);
                if(serialstat_entry != NULL)
                        serialstat_entry->read_proc = serialstat_read;

...
#define EXT_2G_WLAN_GPIO  1  /* E1 */
#define EXT_2G_WLAN_GPIO_BIT (0x1<<EXT_2G_WLAN_GPIO)
#define EXT_5G_WLAN_GPIO  5  /* E5 */
#define EXT_5G_WLAN_GPIO_BIT (0x1<<EXT_5G_WLAN_GPIO)
...
#if defined CONFIG_RTL_8197DL || defined CONFIG_RTL_8197D || defined CONFIG_RTL_8196D || defined CONFIG_RTL_8196E
...
#endif
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
static int proc_read_lanwanbt(struct seq_file *s, void *v)
{
...
#endif
#ifdef USB_LED_BIT
static int proc_read_usbled(struct seq_file *s, void *v)
{
        return 0;
}
#endif
...
static ssize_t proc_write_wps_bt(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return write_wps_bt(file, userbuf,count, off);
}
...
#endif
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
static int write_lanwanbt( struct file *file, const char *buffer, u_long count, void *data )
{
        char buf2[32];
...
static ssize_t proc_write_lanwanbt(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return write_lanwanbt(file, userbuf,count, off);
}
#endif
#if defined CONFIG_RTL_8197DL || defined CONFIG_RTL_8197D || defined CONFIG_RTL_8196D
static int write_usbled( struct file *file, const char *buffer, u_long count, void *data )
{
        if(buffer[0]=='1')
        {
                gpio_shadow = RTL_R32(PABCD_DAT);
...
static ssize_t proc_write_usbled(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return write_usbled(file, userbuf,count, off);
}
#endif
#else
#ifdef CONFIG_RTL_KERNEL_MIPS16_CHAR
...
static int check_ext2g_wps_bt(void)
{
        ...
        gpio_value = RTL_R32(PEFGH_DAT);
        if(gpio_value & EXT_2G_WLAN_GPIO_BIT)
                return 1; /* not pushed */
        else
                return 0; /* pushed */
}
...
static int check_ext5g_wps_bt(void)
{
        ...
        gpio_value = RTL_R32(PEFGH_DAT);
        if(gpio_value & EXT_5G_WLAN_GPIO_BIT)
                return 1; /* not pushed */
        else
                return 0; /* pushed */
}
...
{
        int gpio_val;
        gpio_val = RTL_R32(PEFGH_DAT);
        if(flag) gpio_val &= ~(0x1 << gpio);
        else gpio_val |= (0x1 << gpio);
        RTL_W32(PEFGH_DAT, gpio_val);
        return 0;
}

static int proc_read_led2g_wps_bt(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        char *p = buffer;
        int len;
...
static int proc_write_led2g_wps_bt( struct file *file, const char *buffer, u_long count, void *data )
{
        if(!strncmp(buffer,"on",2))
        {
                set_led_op(EXT_2G_WLAN_GPIO,1);
        }
        else if(!strncmp(buffer,"off",3))
        {
                set_led_op(EXT_2G_WLAN_GPIO,0);
...
static int proc_read_led5g_wps_bt(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        char *p = buffer;
        int len;
...
static int proc_write_led5g_wps_bt( struct file *file, const char *buffer, u_long count, void *data )
{
        if(!strncmp(buffer,"on",2))
        {
                set_led_op(EXT_5G_WLAN_GPIO,1);
        }
        else if(!strncmp(buffer,"off",3))
        {
                set_led_op(EXT_5G_WLAN_GPIO,0);
        }
...
#endif
#if defined CONFIG_RTL_8197DL || defined CONFIG_RTL_8197D || defined CONFIG_RTL_8196D
int usbled_open(struct inode *inode, struct file *file)
{
        return(single_open(file, proc_read_usbled, NULL));
}
#endif
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
int lanwanbt_open(struct inode *inode, struct file *file)
{
        return(single_open(file, proc_read_lanwanbt, NULL));
}
#endif
...
#endif
#if defined CONFIG_RTL_8197DL || defined CONFIG_RTL_8197D || defined CONFIG_RTL_8196D
struct file_operations usbled_proc_fops = {
        .open            = usbled_open,
        .write           = proc_write_usbled,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release         = single_release,
};
#endif
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
struct file_operations lanwanbt_proc_fops = {
        .open            = lanwanbt_open,
        .write           = proc_write_lanwanbt,
        .read            = seq_read,
        .llseek          = seq_lseek,
        .release         = single_release,
};
...
static  int init_iptime_led_spec(void)
{
	...
	sprintf(name, "driver/wps_bt");
        ...
        proc_create_data(name, 0, &proc_root,
                        &usbled_proc_fops, NULL);
#endif
#ifdef CONFIG_EFM_PATCH_MOBILE_AP
        sprintf(name, "driver/lanwan_bt");
        proc_create_data(name, 0, &proc_root,
                        &lanwanbt_proc_fops, NULL);
#endif
#else
        sprintf(name, "driver/reset_bt");
        ...
        if(proc_entry)
        {
                proc_entry->read_proc=&proc_read_led2g_wps_bt;
                proc_entry->write_proc=&proc_write_led2g_wps_bt;
        }
        ...
        if(proc_entry)
        {
                proc_entry->read_proc=&proc_read_led5g_wps_bt;
                proc_entry->write_proc=&proc_write_led5g_wps_bt;
        }
#endif

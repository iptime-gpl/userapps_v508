...
int (*nf_nat_bnet_hook)(struct sk_buff *skb,
...
static int bnet_help(struct sk_buff *skb, unsigned int protoff,
		     struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
        ...
        else
	{
		...
		if (nf_nat_bnet)
		{
			...
			ret = nf_nat_bnet(skb, ct, ctinfo, protoff, iph, bnet_data);
		}
		else

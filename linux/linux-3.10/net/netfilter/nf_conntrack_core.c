...
#else
static int proc_read_conntrack_info(struct seq_file *seq, void *v);
static int proc_open_conntrack_info(struct inode *inode, struct file *file);
#endif
#endif
...
#else
static int proc_read_conntrack_ip_stat(struct seq_file *seq, void *v);
static int proc_open_conntrack_ip_stat(struct inode *inode, struct file *file);
#endif
...
int check_conntrack_ratio_in_dnat(struct nf_conn *ct, const struct nf_nat_ipv4_multi_range_compat *mr)
{
        int ipidx;
		...
		struct sk_buff *skb)
{
	...
	#endif
	{
...
#ifdef CONFIG_EFM_PATCH_XXX
        {
                ...
                if(ipv4_is_multicast(iph->daddr)) FOE_ALG(skb)=1;
        }
...
#else
#ifdef CONFIG_IP_CONNTRACK_LIMIT_CONTROL
static struct file_operations conntrack_info_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_conntrack_info,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release
};
#endif
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
static struct file_operations conntrack_ip_stat_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_conntrack_ip_stat,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release
};
...
#else
                if(!(proc = proc_create("net/conntrack_info", 0, 0, &conntrack_info_fops)))
                        printk("!! FAIL to create net/conntrack_info PROC !!\n");
#endif
        }
#endif
#ifdef CONFIG_IP_CONNTRACK_LIMIT_PER_IP
...
#else
                if(!(proc = proc_create("net/conntrack_ip_stat", 0, 0, &conntrack_ip_stat_fops)))
                        printk("!! FAIL to create net/conntrack_ip_stat PROC !!\n");
#endif
...
static int proc_read_conntrack_info(struct seq_file *seq, void *v)
{
        seq_printf(seq, "total:%d(%d), tcp:%d(%d), udp:%d(%d), icmp:%d(%d)",
                        init_net.ct.count.counter,nf_conntrack_max,
                        tcp_conn_count.counter,tcp_conntrack_max,
                        udp_conn_count.counter,udp_conntrack_max,
                        icmp_conn_count.counter,icmp_conntrack_max
...
static int proc_open_conntrack_info(struct inode *inode, struct file *file)
{
        return single_open(file, proc_read_conntrack_info, NULL);
}
#endif
#endif
...
static int proc_read_conntrack_ip_stat(struct seq_file *seq, void *v)
{
        ...
        for( i = 0 ; i < MAX_RATE_IP_NUM; i++ )
                total += conntrack_count_per_ip[i];
        seq_printf(seq, "TOTAL COUNT :%-5d\n", total);
        seq_printf(seq, "LOCAL CPU   :%-5d\n", conntrack_count_per_ip[LANCPU_IPIDX]);
        seq_printf(seq, "WAN CPU     :%-5d\n", conntrack_count_per_ip[WANCPU_IPIDX]);
#if defined(CONFIG_DRIVERLEVEL_REAL_IPCLONE) || defined(CONFIG_DRIVERLEVEL_REAL_IPCLONE_MODULE)
        seq_printf(seq, "TWIN IP WAN1:%-5d\n", conntrack_count_per_ip[WAN1_TWINIP_IDX]);
        seq_printf(seq, "TWIN IP WAN2:%-5d\n", conntrack_count_per_ip[WAN2_TWINIP_IDX]);
#endif
        for( i = 0 ; i < MAX_RATE_IP_NUM; i++ )
        {
                if(!(i%5)) seq_printf(seq, "\n");
                seq_printf(seq, "[%-3d]:%-5d ",i,conntrack_count_per_ip[i]);
        }
...
static int proc_open_conntrack_ip_stat(struct inode *inode, struct file *file)
{
        return single_open(file, proc_read_conntrack_ip_stat, NULL);
}
#endif
#endif

...
static int proc_read_burst_drop(struct seq_file *seq, void *v)
{
        seq_printf(seq, "%d %d", burst_drop_flag, virus_drop_flag);

        return 0;
}

static int write_burst_drop(struct file *file, const char *buffer, u_long count, void *data)
{
        if(buffer[0] == '0' )
                burst_drop_flag= 0;
        else
...
static ssize_t proc_write_burst_drop(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return write_burst_drop(file, userbuf,count, off);
}

static int proc_open_burst_drop(struct inode *inode, struct file *file)
{
        return single_open(file, proc_read_burst_drop, NULL);
}

struct file_operations burst_drop_fops = {
        .open            = proc_open_burst_drop,
        .write           = proc_write_burst_drop,
        .read            = seq_read,
        .llseek          = seq_lseek,
...
static u32 init_net_detect_proc(void)
{
	...
	proc_entry = proc_create("netdetect/burst_drop", 0, 0, &burst_drop_fops);

...
#ifdef CONFIG_SIP_UDP_TIMEOUT_PATCH
		      ...
		      unsigned int *timeouts)
{
	/* If we've seen traffic both ways, this is some kind of UDP
	   stream.  Extend timeout. */
        ...
        if ( (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all == htons(SIP_UDP_PORT)) ||
                (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all == htons(WYZ070_SMS_UDP_PORT)) )
        {
                nf_ct_refresh_acct(ct, ctinfo, skb, CONFIG_SIP_UDP_TIMEOUT_PATCH * HZ);
        } else
#endif

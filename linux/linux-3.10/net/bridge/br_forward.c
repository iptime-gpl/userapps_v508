...
static inline int should_deliver(struct net_bridge_port *p, struct sk_buff *skb)
{
        if (skb->dev == p->dev ||
            p->state != BR_STATE_FORWARDING)
                return 0;
        if(!p->port_map || !br_port_get_rcu(skb->dev))
                return 1;
        if(p->port_map & (1<<(br_port_get_rcu(skb->dev)->port_no-1)))
                return 1;
        else
        {
#if defined CONFIG_EFM_PATCH && defined CONFIG_RTL_HARDWARE_NAT
                if(p->br && (!memcmp(eth_hdr(skb)->h_source,p->br->dev->dev_addr,6)))
                {
                        return 1;
                }
#endif

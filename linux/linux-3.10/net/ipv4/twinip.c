...
void twinip_rx(struct sk_buff *skb)
{
	...
	{
#if defined(CONFIG_EFM_PATCH) && defined(CONFIG_RTL_819X)
		if (!strcmp(skb->dev->name, CONFIG_LOCAL_IFNAME) || !strcmp(skb->dev->name, "eth0"))
#else
		if (!strcmp(skb->dev->name, CONFIG_LOCAL_IFNAME))
			...
			if(htons(ethh->h_proto) == ETH_P_IP){
				iph = ip_hdr(skb);
				if(iph)
					printk("[twinip_rx func => skb->dev->name] [%s] %x => %x\n", skb->dev->name, iph->saddr, iph->daddr);
			}
			printk("[twinip_rx func => skb->dev->dev_addr] %x:%x:%x:%x:%x:%x\n"
				, skb->dev->dev_addr[0]
				, skb->dev->dev_addr[1]
				, skb->dev->dev_addr[2]
				, skb->dev->dev_addr[3]
			...
			printk("[twinip_rx func => skb->dev ip] %x\n", get_dev_ip(skb->dev));
		}
#endif
        }
}
...
void twinip_tx(struct sk_buff *skb, char *ifname)
{
        ...
        {
#if defined(CONFIG_EFM_PATCH) && defined(CONFIG_RTL_819X)
		if (!strcmp(ifname, CONFIG_LOCAL_IFNAME) || !strcmp(ifname, "eth0"))
#else
		if (!strcmp(ifname, CONFIG_LOCAL_IFNAME))
			...
			if(htons(ethh->h_proto) == ETH_P_IP){
				iph = ip_hdr(skb);
				if(iph)
					printk("[twinip_tx func => ifname] [%s] %x => %x\n", ifname, iph->saddr, iph->daddr);
			}
		}
#endif
        }
...
#else
static int proc_read_ipclone(struct seq_file *seq, void *v)
{
...
static int proc_open_ipclone(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_ipclone, NULL);
}
#endif
...
#else
static int proc_read_wanip(struct seq_file *seq, void *v)
{
        seq_printf(seq, "%08x %08x", ipclone_wanip[0], ipclone_wanip[1] );
	return 0;
}
static int proc_open_wanip(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_wanip, NULL);
}
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int proc_read_twinip(char *page, char **start, off_t off, int count, int *eof, void *data_unused)
{
	char *p;
...
#else
static int proc_read_twinip(struct seq_file *seq, void *v)
{
        if (twinip_rx_handler)
	{
                seq_printf(seq, "1");
		seq_printf(seq, " %08x", IPCLONE_WAN1_VIRTUAL_IP);
                seq_printf(seq, " %08x", IPCLONE_WAN2_VIRTUAL_IP);
	}
        else
                seq_printf(seq, "0");
...
static int proc_open_twinip(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_twinip, NULL);
}
#endif

static int proc_write_twinip(struct file *file, const char __user * buffer, size_t count, loff_t *ppos)
{
	char buf[64], *buf2;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
#else
static struct file_operations ipclone_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_ipclone,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .write          = proc_write_ipclone,
	.release        = single_release
};


static struct file_operations wanip_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_wanip,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .write          = proc_write_wanip,
	.release        = single_release
};

#ifndef CONFIG_DRIVERLEVEL_REAL_IPCLONE_MODULE
static struct file_operations twinip_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_twinip,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .write          = proc_write_twinip,
	.release        = single_release
};
#endif
#endif
...
#else
        if(!(proc_entry = proc_create(name, 0, 0, &ipclone_fops)))
...
#else
        if(!(proc_entry = proc_create(name, 0, 0, &wanip_fops)))
		printk("!! FAIL to create %s PROC !!\n", name);
#endif


#ifndef CONFIG_DRIVERLEVEL_REAL_IPCLONE_MODULE
        sprintf(name, "driver/twinip");
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
        proc_entry = create_proc_entry(name,0,0);
        if(proc_entry)
        {
                proc_entry->read_proc = (read_proc_t*)&proc_read_twinip;
                proc_entry->write_proc = (write_proc_t*)&proc_write_twinip;
        }
#else
        if(!(proc_entry = proc_create(name, 0, 0, &twinip_fops)))
		printk("!! FAIL to create %s PROC !!\n", name);
#endif
#endif
...
#ifndef CONFIG_DRIVERLEVEL_REAL_IPCLONE_MODULE
	remove_proc_entry("driver/twinip", NULL );
#endif
	return 0;
}

...
static int proc_write_smart_qos(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
...
#else
static int proc_read_smart_qos(struct seq_file *seq, void *v)
{
	int i;

	seq_printf(seq, "Smart QoS : %s\n\n", (smart_qos_mon_callback) ? "On" : "Off");
...
static int proc_open_smart_qos(struct inode *inode, struct file *file)
{
        return single_open(file, proc_read_smart_qos, NULL);
}

static struct file_operations smart_qos_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_smart_qos,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .write          = proc_write_smart_qos,
...
#else
	if(!(proc_entry = proc_create("smart_qos", 0, 0, &smart_qos_fops)))
                printk("!! FAIL to create smart_qos PROC !!\n");
#endif
	return 0;
}

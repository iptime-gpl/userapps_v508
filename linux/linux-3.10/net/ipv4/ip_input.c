...
#ifdef CONFIG_EFM_PATCH_SKIP_NF_INPUT_OUTPUT
int sysctl_nf_skip_input_output_flag= 0;
extern int check_skip_packet(int direction, struct sk_buff *skb);
#endif
...
#else
#ifdef CONFIG_EFM_PATCH_SKIP_NF_INPUT_OUTPUT
        if(sysctl_nf_skip_input_output_flag && check_skip_packet(0,skb))
                return ip_local_deliver_finish(skb);
#endif

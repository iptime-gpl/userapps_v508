...
static void convert_data(char *data, u32 *ip, u32 *mask, u16 *proto, u16 *dport)
{
	...
	*mask = htonl(*mask);
	...
	//printk("==> ip:%08x, maks:%08x, proto:%x, dport %x\n", *ip, *mask, *proto, *dport);
}
...
static int kill_ct_original(struct nf_conn *i, void *data)
{
	...
	//printk("=====> org (%d) : %x \n", rst, i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip);
...
static int cleanup_proc_write( struct file *file, const char *buffer, u_long count, void *data )
{
	struct net *net;
	char buf[1024];
	...
	for_each_net(net)
	{
		nf_ct_iterate_cleanup(net, kill_ct_original, (void *)buf);
		nf_ct_iterate_cleanup(net, kill_ct_reply, (void *)buf);
	}
	return count;
}
...
int cleanup_conntrack_open(struct inode *inode, struct file *file)
{
        return(single_open(file, efm_common_read_proc, NULL));
}

static ssize_t cleanup_conntrack_write(struct file * file, const char __user * userbuf,
                     size_t count, loff_t * off)
{
        return cleanup_proc_write(file, userbuf,count, off);
}

struct file_operations cleanup_conntrack_entry = {
        .open            = cleanup_conntrack_open,
        .write           = cleanup_conntrack_write,
        .read            = seq_read,
        .llseek          = seq_lseek,
...
#ifdef CONFIG_EFM_PATCH
	proc_create_data("ctproc/cleanup", 0, 0,
                        &cleanup_conntrack_entry, NULL);
#else
        conn_proc = create_proc_entry("cleanup", 0, proc_entry);
        if( conn_proc != NULL )
	{
              conn_proc->write_proc = &cleanup_proc_write;
	}
#endif
	return 0;
}

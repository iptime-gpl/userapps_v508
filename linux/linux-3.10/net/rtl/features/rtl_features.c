...
static int bypass_ports[] = {6112, 5060, 0}; //Bnet, SIP
int bypass_realtek_nat(struct nf_conn *ct)
{
        int i;

        for (i = 0; bypass_ports[i] != 0; i++)
        {
                if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port == bypass_ports[i]) ||
                    (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == bypass_ports[i]) ||
                    (ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u.udp.port == bypass_ports[i]) ||
                    (ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port == bypass_ports[i]))
                        return 1;
        }
        return 0;
}

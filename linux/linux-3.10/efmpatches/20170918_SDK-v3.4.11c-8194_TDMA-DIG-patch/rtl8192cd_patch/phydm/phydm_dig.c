	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("%s CurrentIGI=0x%x, pDM_DigTable->CurIGValue=0x%x\n",
		__FUNCTION__,
		CurrentIGI,
	...
	)
{
	...
	u1Byte						tdma_high_dym_min, tdma_low_dym_min;
	...
	//1 Boundary Decision
	{
		...
		else
		{
			...
			if ((*pDM_Odm->pBandType == ODM_BAND_2_4G) && (pDM_Odm->SupportICType & ODM_RTL8814A)) /* for 2G 8814 */
				...
				dm_dig_min = 0x20;
			else if ((pDM_Odm->SupportICType & ODM_RTL8822B) && pDM_Odm->ExtLNA)
				dm_dig_min = 0x1e;
			else
...
#else
		{
			...
			if (bDFSBand)
			{
				...
				if(rssi_min < dm_dig_min)
					...
					tdma_high_dym_min = rssi_min;

				/*tdma_low_dym_min(tdma_low_rx_gain_min)*/
				tdma_low_dym_min = dm_dig_min;
			}
		}
#endif
	...
	if(!pDM_Odm->bLinked)
	{
		...
		pDM_DigTable->rx_gain_range_min = odm_ForbiddenIGICheck(pDM_Odm, DIG_Dynamic_MIN, CurrentIGI, pDM_DigTable->rx_gain_range_min);
	}
	...
	)
{
	...
	bool						is_high_low_sel = (tdma_dig_state == TDMA_DIG_FIRST_HIGH_STATE)? 0: 1;
...
#if (PHYDM_TDMA_DIG_SUPPORT == 1)
	if ((pDM_Odm->original_dig_restore == 0) && (pDM_Odm->SupportICType & ODM_RTL8197F) && (is_high_low_sel != NORMAL_DIG)) {

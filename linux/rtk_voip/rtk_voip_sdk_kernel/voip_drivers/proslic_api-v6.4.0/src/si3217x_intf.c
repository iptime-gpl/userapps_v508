...
**
** $Id: si3217x_intf.c,v 1.1.1.1 2016/11/01 07:34:28 hyrtlac Exp $
**
** SI3217X_Intf.c
...
extern const proslicPatch SI3217X_PATCH_B_PBB5;
extern const proslicPatch SI3217X_PATCH_B_PBB12;
extern const proslicPatch SI3217X_PATCH_B_PBB12HV;
...
int returnCode;
    ...
    */
    ...
    {
		LOGPRINT("isVerifiedProslic : REG VAL = %02x\n", data);
	}
#endif
...
int Si3217x_PowerUpConverter(proslicChanType_ptr pProslic)
{
    ...
    ** - delay
    */
    WriteReg(pProHW,pProslic->channel,LINEFEED,LF_FWD_OHT);  /* Force out of pwrsave mode if called in error */
    WriteReg(pProHW,pProslic->channel,LINEFEED,LF_OPEN);     /* Ensure open line before powering up converter */
    reg = ReadReg(pProHW,pProslic->channel,ENHANCE);
    WriteReg(pProHW,pProslic->channel,ENHANCE,reg&0x07);  /* Disable powersave mode */
    ...
    if(vbat < (vbath / 2)) {
        ...
        WriteReg(pProHW,pProslic->channel,ENHANCE,reg);   /* Restore ENHANCE */
        Delay(pProTimer,50);
    }
    ...
    */
    ...
    if(pProslic->debugMode)
    {
        LOGPRINT ("Si3217x : Channel %d : VBAT Up = %d.%d v\n",pProslic->channel,(int)((vbat/SCALE_V_MADC)/1000), (int)(((vbat/SCALE_V_MADC) - (vbat/SCALE_V_MADC)/1000*1000)));
    }
#endif
...
int Si3217x_Init_with_Options (proslicChanType_ptr *pProslic, int size, initOptionsType init_opt){
    ...
    */
	...
	{
		/*
		** Load patch (load on every channel)
		*/
		for (k=0;k<size;k++)
		{
			if ((pProslic[k]->channelEnable)&&(pProslic[k]->channelType == PROSLIC))
			{
				...
				{
	#ifdef SIVOICE_MULTI_BOM_SUPPORT
					if(Si3217x_General_Configuration.bomOpt == BO_DCDC_FLYBACK)
					{
						patch = &(SI3217X_PATCH_B_FLBK);
					}
					else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_BUCK_BOOST)
					{
						patch = &(SI3217X_PATCH_B_BKBT);
					}
					else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_5V)
					{
						patch = &(SI3217X_PATCH_B_PBB5);
					}
					else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V)
					{
						patch = &(SI3217X_PATCH_B_PBB12);
					}
					else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V_HV)
					{
						patch = &(SI3217X_PATCH_B_PBB12HV);
					}
					else
					{
	#ifdef ENABLE_DEBUG
						if (pProslic[k]->debugMode)
						{
							LOGPRINT("si3217x : Channel %d : Invalid Patch\n",pProslic[k]->channel);
						}
	#endif
						pProslic[k]->channelEnable = 0;
	...
	#ifdef DISABLE_VERIFY_PATCH
	#else
					data = Si3217x_VerifyPatch(pProslic[k],patch);
					if (data){
						pProslic[k]->channelEnable=0;
				...
				else
				{
					...
					if (pProslic[k]->debugMode)
					{
						LOGPRINT("si3217x : Channel %d : Unsupported Device Revision (%d)\n",pProslic[k]->channel,pProslic[k]->deviceId->chipRev );
					}
	#endif
					pProslic[k]->channelEnable = 0;
    ...
    */
	...
	if(init_opt != INIT_NO_CAL)
	{
		...
		*/
		calSetup[1] = SI3217X_CAL_STD_CALR1;
		calSetup[2] = SI3217X_CAL_STD_CALR2;
...
int num_reinit_chan = 1;
	...
	WriteReg(pProHW,pProslic->channel,RESET,0x01);
	Delay(pProTimer,100);
...
int Si3217x_RingSetup (proslicChanType *pProslic, int preset){
	...
	{
		WriteRAM(pProHW,pProslic->channel,VBATR_EXPECT,Si3217x_General_Configuration.vbatr_expect);
    #ifdef ENABLE_DEBUG
		if(pProslic->debugMode)
		{
...
#define DCDC_RNGTYPE_BKBT     0L
    ...
    if(Si3217x_General_Configuration.bomOpt == BO_DCDC_BUCK_BOOST) 
    {
        WriteRAM(pProHW,pProslic->channel,DCDC_RNGTYPE,DCDC_RNGTYPE_BKBT);
    }
#endif
...
*/
    ...
    if (gain > GAIN_MAX)
	{
		...
		if(pProslic->debugMode)
		{
			LOGPRINT("ProSLIC : Si3217x : dbgSetGain : Gain %d out of range\n",gain);
		}
#endif
...
*/
    ...
    if (gain > (GAIN_MAX*10L))
	{
		...
		if(pProslic->debugMode)
		{
			LOGPRINT("ProSLIC : Si3217x : dbgSetGain : Gain %d dB*10 out of range\n",gain);
		}
#endif
...
** uInt32 - RAM contents
*/
ramData Si3217x_ReadRAM (proslicChanType *pProslic, uInt16 addr)
{
	return (ReadRAM(pProHW,pProslic->channel, addr));
}

/*
** Function: Si3217x_WriteRAM
**
** Description: 
** Allows direct SPI access at ProSLIC layer
** Channel embeded in channel obj, so it is not passed into this func
...
** RC_NONE
*/
int Si3217x_WriteRAM (proslicChanType *pProslic, uInt16 addr, ramData data)
{

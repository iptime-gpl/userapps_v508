 ...
 *
 * $Revision: 1.1.1.1 $
 * $LastChangedDate: 2013-08-14 20:34:17 -0500 (Wed, 14 Aug 2013) $
 */
    ...
    void                    *value)
{
    ...
    switch (option) {
        ...
        case VP_OPTION_ID_DCFEED_PARAMS: {
            ...
            bool updateDcFeed = FALSE;
            bool updateLoopSupervision = FALSE;
            bool loopSupervisionUpdate = FALSE;
            ...
            /* DO NOT change VOC. Only support 48V */

            /* Set ILA if the mask is set */
            if (dcFeedParams.validMask & VP_OPTION_CFG_ILA) {
                /* Set bool so DC Feed Register will be updated now */
                updateDcFeed = TRUE;
                ...
                pLineObj->calLineData.dcFeedRef[VP890_ILA_INDEX] |=
                    ((uint8)dcFeedParams.ila & VP890_ILA_MASK);
            }
            /* Set the GKey Threshold if the mask is set */
            if (dcFeedParams.validMask & VP_OPTION_CFG_GKEY_THRESHOLD) {
                int32 error;

                /* Set bool so Loop Supervision Register will be updated now */
                updateLoopSupervision = TRUE;
                ...
                if (dcFeedParams.gkeyThreshold > 42) {
                    dcFeedParams.gkeyThreshold = 42;
                }
                dcFeedParams.gkeyThreshold /= 6;

                /* Shift into silicon location */
                dcFeedParams.gkeyThreshold = ((uint16)dcFeedParams.gkeyThreshold << 3) &
                    VP890_GKEY_THRESH_MASK;
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_THRESH_BYTE] &=
                    ~VP890_GKEY_THRESH_MASK;
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_THRESH_BYTE] |=
                    ((uint8)dcFeedParams.gkeyThreshold & VP890_GKEY_THRESH_MASK);
            }
            ...
            if (dcFeedParams.validMask & VP_OPTION_CFG_HOOK_THRESHOLD) {
                ...
                if (dcFeedParams.hookThreshold > 15) {
                    dcFeedParams.hookThreshold = 15;
                }
                ...
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_THRESH_BYTE] &=
                    ~VP890_SWHOOK_THRESH_MASK;
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_THRESH_BYTE] |=
                    ((uint8)dcFeedParams.hookThreshold & VP890_SWHOOK_THRESH_MASK);

                VpMemCpy(pLineObj->loopSup, pLineObj->calLineData.loopSup, VP890_LOOP_SUP_LEN);
                loopSupervisionUpdate = TRUE; /* Stop future updates */

                /* Adjust for hysterisis if off-hook */
                if ((VpCslacLineCondType)(pLineObj->lineState.condition & VP_CSLAC_HOOK)
                    == VP_CSLAC_HOOK) {
                    if ((pLineObj->loopSup[VP890_LOOP_SUP_THRESH_BYTE] & VP890_SWHOOK_THRESH_MASK)
                        >= pLineObj->hookHysteresis) {
                        pLineObj->loopSup[VP890_LOOP_SUP_THRESH_BYTE] -= pLineObj->hookHysteresis;
                    } else {
            ...
            if (updateLoopSupervision) {
                if (!(loopSupervisionUpdate)) {
                    VpMemCpy(pLineObj->loopSup, pLineObj->calLineData.loopSup, VP890_LOOP_SUP_LEN);
                }
                VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP890_LOOP_SUP_WRT,
                    VP890_LOOP_SUP_LEN, pLineObj->loopSup);
            }
            ...
            if (dcFeedParams.validMask & VP_OPTION_CFG_BATT_FLOOR) {
                ...
                error = dcFeedParams.battFloor % 5;
                if (error > 2) {
                ...
                pDevObj->devProfileData.swParams[VP890_FLOOR_VOLTAGE_BYTE] &= ~VP890_FLOOR_VOLTAGE_MASK;
                pDevObj->devProfileData.swParams[VP890_FLOOR_VOLTAGE_BYTE] |=
                    ((uint8)dcFeedParams.battFloor & VP890_FLOOR_VOLTAGE_MASK);
                if ((pDevObj->stateInt &= (VP890_LINE0_LP | VP890_LINE1_LP)) == 0) {
                    /* Write this ONLY if the device is NOT in LPM */
                    VpMpiCmdWrapper(deviceId, ecVal, VP890_REGULATOR_PARAM_WRT,
        ...
        case VP_OPTION_ID_RINGING_PARAMS: {
            ...
            if ((ringingParams.validMask & VP_OPTION_CFG_FREQUENCY)
                ...
                if (isTrap) {
                    ...
                    ringingValues[VP890_SIGB_FREQ_MSB] =
                        (uint8)((uint16)(ringingParams.frequency >> 8) & 0x00FF);
                    ringingValues[VP890_SIGB_FREQ_LSB] =
                    ...
                    ringingParams.frequency /= 3662; /* VP890_GENA_FREQ_STEPSIZE; */
                    ringingValues[VP890_SIGA_FREQ_MSB] =
                        (uint8)((ringingParams.frequency >> 8) & 0x00FF);
                    ringingValues[VP890_SIGA_FREQ_LSB] =
                        (uint8)((ringingParams.frequency) & 0x00FF);

                    /* Sine - no rise time allowed */
                    ringingValues[VP890_SIGB_FREQ_MSB] = 0;
                    ringingValues[VP890_SIGB_FREQ_LSB] = 0;
                }
            }
            ...
            if ((ringingParams.validMask & VP_OPTION_CFG_AMPLITUDE)
                ...
                ringingParams.amplitude /= 4730; /* VP890_RINGING_AMP_SCALE; */
                ringingValues[VP890_SIGA_AMP_MSB] =
                    (uint8)((ringingParams.amplitude >> 8) & 0x00FF);
                ringingValues[VP890_SIGA_AMP_LSB] =
            ...
            if ((ringingParams.validMask & VP_OPTION_CFG_DC_BIAS)
                ...
                ringingParams.dcBias /= 4712; /* VP890_RINGING_BIAS_SCALE; */
                ringingValues[VP890_SIGA_BIAS_MSB] = (uint8)((ringingParams.dcBias >> 8) & 0x00FF);
                ringingValues[VP890_SIGA_BIAS_LSB] = (uint8)((ringingParams.dcBias) & 0x00FF);
            }
            ...
            if ((ringingParams.validMask & VP_OPTION_CFG_RINGTRIP_THRESHOLD)
                ...
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_RT_MODE_BYTE] &=
                    ~VP890_LOOP_SUP_RT_MODE_THRSHLD;
                pLineObj->calLineData.loopSup[VP890_LOOP_SUP_RT_MODE_BYTE] |=
                    ((uint8)ringingParams.ringTripThreshold & VP890_LOOP_SUP_RT_MODE_THRSHLD);
            }
            ...
            if (updateRingReq) {
                ...
                VpMemCpy(pLineObj->ringingParamsRef, ringingValues, VP890_RINGER_PARAMS_LEN);
            ...
            if (updateLoopSupervisionReq) {
                ...
                /* We didn't change the hook threshold here, so go ahead and write the data */
                VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP890_LOOP_SUP_WRT,
                    VP890_LOOP_SUP_LEN, pLineObj->loopSup);
            }
        } break;

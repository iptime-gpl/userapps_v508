...
**
** $Id: proslic.c,v 1.1.1.1 2016/11/01 07:34:28 hyrtlac Exp $
**
** Proslic.c
**
** Author(s): 
...
#ifdef SI324X
	if (hProslic->deviceId->chipType >= SI3240 && hProslic->deviceId->chipType <= SI3247)
		return Si324x_GetPLLFreeRunStatus (hProslic);
#endif
#ifdef SI322X
	if (hProslic->deviceId->chipType >= SI3226 && hProslic->deviceId->chipType <= SI3227)
		return Si3226_GetPLLFreeRunStatus (hProslic);
#endif
#ifdef SI321X
	if (hProslic->deviceId->chipType >= SI3210 && hProslic->deviceId->chipType <= SI3216M)
		return Si321x_GetPLLFreeRunStatus (hProslic);
#endif
#ifdef SI3217X
	if (hProslic->deviceId->chipType >= SI32171 && hProslic->deviceId->chipType <= SI32179)
		return Si3217x_GetPLLFreeRunStatus(hProslic);
#endif
#ifdef SI3226X
	if (hProslic->deviceId->chipType >= SI32260 && hProslic->deviceId->chipType <= SI32269)
		return Si3226x_GetPLLFreeRunStatus(hProslic);
#endif
	return 1;
}
...
#ifndef DISABLE_MALLOC
#ifdef REALTEK_PATCH_FOR_SILAB
  	*pstnCheckObj = kmalloc(sizeof(proslicDiffPSTNCheckObjType), 0x20);	//GFP_ATOMIC
#else
...
#ifdef SI3226X
    if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
        return Si3226x_GetLinefeedStatus (pProslic, newLinefeed);
#endif
    return 1;
}

int ProSLIC_Set_Ring_Cadence_ON(proslicChanType *pProslic, unsigned short msec, uInt8 timer_enable){
#ifdef SI324X
	if (pProslic->deviceId->chipType >= SI3240 && pProslic->deviceId->chipType <= SI3247)
		Si324x_Set_Ring_Cadence_ON(pProslic, msec, timer_enable);
#endif
#ifdef SI322X
	if (pProslic->deviceId->chipType >= SI3226 && pProslic->deviceId->chipType <= SI3227)
		Si3226_Set_Ring_Cadence_ON(pProslic, msec, timer_enable);
#endif
#ifdef SI3217X
	if (pProslic->deviceId->chipType >= SI32171 && pProslic->deviceId->chipType <= SI32179)
		Si3217x_Set_Ring_Cadence_ON(pProslic, msec, timer_enable);
#endif
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		Si3226x_Set_Ring_Cadence_ON(pProslic, msec, timer_enable);
#endif
	return 1;

}

int ProSLIC_Set_Ring_Cadence_OFF(proslicChanType *pProslic, unsigned short msec, uInt8 timer_enable){
#ifdef SI324X
	if (pProslic->deviceId->chipType >= SI3240 && pProslic->deviceId->chipType <= SI3247)
		Si324x_Set_Ring_Cadence_OFF(pProslic, msec, timer_enable);
#endif
#ifdef SI322X
	if (pProslic->deviceId->chipType >= SI3226 && pProslic->deviceId->chipType <= SI3227)
		Si3226_Set_Ring_Cadence_OFF(pProslic, msec, timer_enable);
#endif
#ifdef SI3217X
	if (pProslic->deviceId->chipType >= SI32171 && pProslic->deviceId->chipType <= SI32179)
		Si3217x_Set_Ring_Cadence_OFF(pProslic, msec, timer_enable);
#endif
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		Si3226x_Set_Ring_Cadence_OFF(pProslic, msec, timer_enable);
#endif
	return 1;
}
...
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		Si3226x_Set_Impendance_Silicon(pProslic, country, impd);
#endif
	return 1;
}
...
#ifdef SI324X
	#error "API ProSLIC_Set_Impendance is not supported for Si3226x."
#endif
#ifdef SI322X
	if (pProslic->deviceId->chipType >= SI3226 && pProslic->deviceId->chipType <= SI3227)
...
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		Si3226x_Set_Impendance(pProslic, preset);
#endif
	return 1;
}
...
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		return Si3226x_SendNTTCAR_check(chid, pProslic, time_out);
#endif
	return 1;
}
...
#ifdef SI3226X
	if (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269)
		return SI3226x_SetUserMode(pProslic, on);
#endif
	return 1;
}
...
#else
	PRINT_R("API ProSLIC_SO_DTRx_Loopback is not supported for this SLIC.");
#endif
	return 1;
}

...
**
** $Id: vdaa.c,v 1.1.1.1 2016/11/01 07:34:28 hyrtlac Exp $
**
** Vdaa.c
...
#ifdef REALTEK_PATCH_FOR_SILAB
extern vdaa_Hybrid_Cfg2 Vdaa_Hybrid_Presets2 [];

int Vdaa_HybridSetup2 (vdaaChanType *pVdaa,int32 preset){
	uInt8 regTemp = 0;
	uInt8 regSave = 0;
	...
	regTemp = ReadReg(pVdaaHW,pVdaa->channel,AC_TERM_CTRL) & 0xF0;
	regTemp |= Vdaa_Hybrid_Presets2[preset].acim;
	WriteReg(pVdaaHW,pVdaa->channel,AC_TERM_CTRL,regTemp);

	WriteReg(pVdaaHW,pVdaa->channel,HYB1,Vdaa_Hybrid_Presets2[preset].hyb1);
	WriteReg(pVdaaHW,pVdaa->channel,HYB2,Vdaa_Hybrid_Presets2[preset].hyb2);
	WriteReg(pVdaaHW,pVdaa->channel,HYB3,Vdaa_Hybrid_Presets2[preset].hyb3);
	WriteReg(pVdaaHW,pVdaa->channel,HYB4,Vdaa_Hybrid_Presets2[preset].hyb4);
	WriteReg(pVdaaHW,pVdaa->channel,HYB5,Vdaa_Hybrid_Presets2[preset].hyb5);
	WriteReg(pVdaaHW,pVdaa->channel,HYB6,Vdaa_Hybrid_Presets2[preset].hyb6);
	WriteReg(pVdaaHW,pVdaa->channel,HYB7,Vdaa_Hybrid_Presets2[preset].hyb7);
	WriteReg(pVdaaHW,pVdaa->channel,HYB8,Vdaa_Hybrid_Presets2[preset].hyb8);

	WriteReg(pVdaaHW,pVdaa->channel,CTRL2,regSave); /* Restore hybrid enable state at entry */

	//printk("Set FXO Hybrid to group %d\n", preset);
...
#ifdef REALTEK_PATCH_FOR_SILAB//Fix Drvier Bug
    return (ReadReg(pVdaaHW,pVdaa->channel,LSIDE_STAT) & 0x40);
#else    
...
#ifdef ENABLE_DEBUG
	if (pVdaa->debugMode)
		LOGPRINT("VDAA PowerupLineside, channe%d \n",pVdaa->channel);
#endif
#endif
